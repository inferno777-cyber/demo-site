...
salebtn.addEventListener("click",function(){
    ...
    inputt.classList.add("inputt");
    acc.type="number";
    acc.placeholder="accessories";
    acc.classList.add("acc");
    salein.appendChild(sel);
    salein.appendChild(inputt);
    salein.appendChild(acc);
    ...
    sub.addEventListener("click",function(){
        ...
        accbox.push(acc.value);
        let mum=accbox.map(Number);
        result=mum.reduce((acc,n)=>acc+n,0);
    }
...
generate.addEventListener("click",function(){
    ...
    let an=document.getElementById("an");
       ...
       container.forEach((n,index)=>{
        ...
        document.getElementById("a").innerText=result+"Ks";
        let sum=Math.floor(answer+result);
        document.getElementById("ana").innerText=sum+"Ks";
    });   
